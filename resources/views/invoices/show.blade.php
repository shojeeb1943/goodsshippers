                    ...
                    @if(in_array($invoice->status, ['draft', 'sent']))
                        <div class="bg-white p-5 rounded-xl border border-indigo-100 shadow-sm">
                            ...
                            <form action="{{ route('payment.initiate', $invoice) }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full inline-flex justify-center items-center px-4 py-2.5 bg-accent hover:bg-accent/90 text-white font-bold rounded-lg shadow-sm shadow-accent/30 transition-colors">
